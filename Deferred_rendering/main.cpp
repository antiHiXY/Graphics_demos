...
void RenderCube(unsigned int VBO, unsigned int cubeVAO);
void RenderLightCube(unsigned int VBO, unsigned int lightCubeVAO);
...
int main()
{
    ...
    geometryPassShader.LoadShader("Shaders/g_buffer.vs", "Shaders/g_buffer.fs");
    lightPassShader.LoadShader("Shaders/deferred_shading.vs", "Shaders/deferred_shading.fs");
    lightCubeShader.LoadShader("Shaders/light_cube.vs", "Shaders/light_cube.fs");

    // configure cube VAO (and VBO)
    unsigned int cubeVBO = 0, cubeVAO = 0;
    // configure light cube VAO
    ...
    for (unsigned int i = 0; i < NR_LIGHTS; i++)
    {
        ...
        float xPos = ((rand() % 100) / 100.0) * 4.0 - 2.0;
        float yPos = ((rand() % 100) / 100.0) * 6.0 - 3.0;
        float zPos = ((rand() % 100) / 100.0) * 4.0 - 2.0;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            lightPassShader.SetVec3f("lights[" + std::to_string(i) + "].Position", lightPositions[i]);
            lightPassShader.SetVec3f("lights[" + std::to_string(i) + "].Color", lightColors[i]);
        }
        lightPassShader.SetVec3f("viewPos", camera.m_Position);
        // render quad
        ...
        // 4. render lights on top of the scene
        lightCubeShader.Use();
        ...
        for (unsigned int i = 0; i < lightPositions.size(); i++)
        {
            ...
            lightCubeShader.SetVec3f("lightColor", lightColors[i]);
            RenderLightCube(cubeVBO, lightCubeVAO);
        }
